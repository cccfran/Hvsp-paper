...
HCD <- function(A,a,K,BH=2,adj=TRUE,plot.it=FALSE,lap=FALSE,reg=FALSE,n.min=25,D=NULL, varimax = F){
  f <- A
  ...
  if(varimax) {
    print("VSP")
    clusters <- break.cl.vsp(f, a, xi.loc.labels, ncl, 1:n,BH,lap=lap,reg=reg,n.min,D=D)
  } else {
...
## cl.labels is the current node index involved in the function call
break.cl.vsp = function(f, per.a, xi.loc.labels, ncl, cl.labels,BH=2,lap=FALSE,reg=FALSE,n.min=25,D=NULL) {
  nisol = which(rowSums(f) > 0)
  isol = which(rowSums(f) == 0)
  if((length(nisol)<=8)||(length(isol)>=5*length(nisol))||(length(nisol)<2*n.min)){
    ncl = ncl + 1
  ...
  if(split.flag) {
    if(lap){
      ...
      eig.nf <- vsp(f, rank = 2, center = F, degree_normalize = F)
...
SpectralClust.varimax <- function(A,a,K){
  graph.f <- graph.adjacency(A,mode="undirected")
  A <- get.adjacency(graph.f, sparse = T)
  f <- A
  all.deg <- colSums(f)
  ...
  #   }
  # }
  vsp_ret <- vsp(f, rank = K, center = F)
  clustering <- apply(vsp_ret$Y, 1, function(x) which.max(abs(x)))
  clus = list(clustering=clustering)
